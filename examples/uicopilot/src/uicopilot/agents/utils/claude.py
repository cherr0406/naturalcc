...
def claude(prompt: str, image: Image.Image, text: str, api_key: str | None = None):
    buffered = io.BytesIO()
    image.save(buffered, format="PNG")
    img_str = buffered.getvalue()
    image_base64 = base64.b64encode(img_str).decode('utf-8')
    # Prepare the content list for the API
    user_message: list[MessageParam] = [
        {
            "role": "user",
            "content": [
                {
    ...
    client = anthropic.Anthropic(api_key=api_key if api_key else os.environ["API_KEY_CLAUDE"])
    ...
    # 提取并返回 HTML 内容
    if message and isinstance(message.content, list) and len(message.content) > 0 and isinstance(message.content[0], dict):
        html_content = message.content[0].get("text", "")
...
response = claude(prompt, test_image, text, os.environ["API_KEY_CLAUDE"])
